...
int main(void)
{
   ...
   print_uname();
    ...
    threadParams[0].sequencePeriods=LCM_PERIOD;
...
void *Sequencer(void *threadp)
{
	...
	current_realtime = current_realtime + ((double)delay_time.tv_nsec / (double)NANOSEC_PER_SEC);
        ...
        // Service_3 = RT_MAX-3	@ 14.2857 Hz
        if((seqCnt % 7) == 0) sem_post(&semS3);

        // Service_4 = RT_MAX-4	@ 7.6923 Hz
...
void *Service_1(void *threadp)
{
    ...
    while(!abortS1)
    {
        ...
        clock_gettime(MY_CLOCK_TYPE, &current_time_val); current_realtime=realtime(&current_time_val);
        syslog(LOG_CRIT, "[COURSE:%d][ASSIGNMENT:%d]: Thread %d start X @ %lf on core %d\n", COURSE, ASSIGNMENT, threadParams->threadIdx, current_realtime, sched_getcpu());
    }
...
void print_uname(void)
{
    ...
    if (fgets(uname_buffer, sizeof(uname_buffer), uname) != NULL)
    {
        ...
        syslog(LOG_CRIT, "[COURSE:%d][ASSIGNMENT:%d]: %s", COURSE, ASSIGNMENT, uname_buffer);
    }
    pclose(uname);
}
